...
size_t getNewVecCapacity(Vector* vec) {
  return (vec->_capacity == 0) ? VEC_BASE_CAPACITY
                               : vec->_capacity * 2;
}
...
void* vec_get_raw(Vector* vec, size_t index) {
  if (index >= vec->Len) {
    fprintf(
      stderr,
      "[Vector] Index out of bounds (%zu >= %zu)\n",
      index,
      vec->Len
    );
    return NULL;
...
void vec_append_raw(Vector* dst, const Vector* src) {
  if (dst == src) {
    fprintf(
      stderr,
      "[Vector] Append failed: self append not allowed\n"
    );
    return;
  }
  ...
  if (dst->_elem_size != src->_elem_size) {
    fprintf(
      stderr,
      "[Vector] Append failed: element size mismatch\n"
    );
    return;
  }
...
void vec_remove(Vector* vec, size_t index) {
  if (index >= vec->Len) {
    fprintf(
      stderr,
      "[Vector] RemoveAt index out of bounds (%zu >= %zu)\n",
      index,
      vec->Len
    );
    return;
