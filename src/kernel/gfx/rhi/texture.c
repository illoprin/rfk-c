...
static void rhi_tex_set_params(
  int target, rhi_TextureFilter filterMin,
  rhi_TextureFilter filterMag, rhi_TextureWrap wrap
) {
...
void rhi_tex_init(rhi_Texture* t, rhi_TextureType type) {
  ...
  *t = (rhi_Texture){0};
  ...
  LogInfo(
    "texture [ID = %d, Type = 0x%04x] created ",
    t->ID,
    t->type
  );
}
...
void rhi_tex2d_alloc(
  rhi_Texture* t, rhi_TextureConfig conf, void* pix
) {
  if (t == NULL) { return; }
  if (t->ID == 0) { return; }
...
void rhi_tex3d_alloc(
  ...
  if (t->type != GL_TEXTURE_3D && t->type != GL_TEXTURE_2D_ARRAY) {
    return;
  }
