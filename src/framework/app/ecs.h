...
struct ECSContext;
typedef void (*SystemUpdateFunc)(
  struct ECSContext* ctx, EntityID entity
);
...
ComponentMask ecs_register_component(
  ECSContext* ctx, uint8_t comp_id, size_t size
);

void* ecs_get_component(
  ECSContext* ctx, EntityID entity, uint8_t comp_id
);
